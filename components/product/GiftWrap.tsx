...
export default function GiftWrap({ onToggle }: { onToggle?: (enabled: boolean, msg: string) => void }) {
  ...
  return (
    <div className={`border-2 rounded-2xl p-4 transition-all duration-300 ${enabled ? 'border-primary-400 bg-primary-50' : 'border-primary-100 bg-white hover:border-primary-200'}`}>
      <label className="flex items-center gap-3 cursor-pointer select-none">
        <div
          onClick={toggle}
          className={`relative w-12 h-6 rounded-full transition-all duration-300 flex-shrink-0 ${enabled ? 'bg-primary-600' : 'bg-primary-200'}`}
        >
      ...
      {enabled && (
        <div className="mt-3 animate-[fadeUp_0.3s_ease]">
          <textarea
            ...
            rows={2}
            className="w-full border border-primary-200 rounded-xl px-3 py-2 text-xs bg-white text-primary-800 focus:outline-none focus:border-primary-400 resize-none"
          />
        </div>
      )}
    </div>
